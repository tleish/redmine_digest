module DigestRulesHelper
  ...
  def event_type_color(event_type)
    case event_type
      when DigestEvent::STATUS_CHANGED then
        '#defff1'
      when DigestEvent::PERCENT_CHANGED then
        '#d8d8ff'
      when DigestEvent::ASSIGNEE_CHANGED then
        '#fbe3ff'
      when DigestEvent::VERSION_CHANGED then
        '#ffe1e3'
      when DigestEvent::PROJECT_CHANGED then
        '#fffcde'
      when DigestEvent::SUBJECT_CHANGED then
        '#FF8E8E'
      when DigestEvent::OTHER_ATTR_CHANGED then
        '#E7E9E6'
      when DigestEvent::COMMENT_ADDED then
        '#e0ffe1'
      when DigestEvent::ATTACHMENT_ADDED then
        '#DFF3FF'
